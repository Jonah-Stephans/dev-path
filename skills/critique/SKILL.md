...
it critiques**, so a fresh critic every pass costs effectively nothing. **If the session cannot dispatch a
subagent this pass is unavailable**: say so, write nothing, and hand back — the next act is re-running this
skill where a subagent can be had. **Do not critique inline instead.** No exception for a session that did
not build the slice, because the session that cannot dispatch is usually the one that did, and asking it to
sort those two apart hands the judgement to the agent this rule exists to distrust. **`fix_cycles` staying
absent is the right outcome and not a gap** — its absence on a slice carrying code is what Integrate
refuses on, so an uncritiqued slice cannot reach a merge unseen.
