...
| `## Current state` | Survey, **one `- O<n> — <finding>` bullet per Outcome, each saying what it was read off**; Design prunes it, **keeping what a finding it keeps was read off**; **Critique's slice pass, striking a wrong note in place on a confirmed finding whose cause it was — error left visible, never overwritten** | Design, at the prune; **Critique's slice pass, which reads the note against the file it names before it strikes it** |
